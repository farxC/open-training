-- Frozen snapshot: schema_version 13 (post exercise_muscle_groups many-to-many
-- rebuild, before the counting_factor column existed on that join table).
...
-- Exercise 1 is a compound built-in movement with three muscle groups already
-- recorded (post-v13 shape) — the v14 rebuild must preserve all three rows and
-- give each a default counting_factor of 1. Exercise 2 is user-created and
-- shares "biceps" with exercise 1's related "Rosca" to exercise the JOIN fan-out
-- once getMuscleSeriesInRange exists. Exercise 3 is the running seed.
...
INSERT INTO user_meta (key, value) VALUES ('schema_version', '13');
