-- Frozen snapshot: schema_version 11, used to verify the session_exercises backfill
-- (added in schema_version 12) preserves the implicit MIN(id)-per-exercise order,
-- not exercise_id order. Sets for exercise_id 2 are inserted BEFORE exercise_id 1's
-- sets even though exercise_id 1 is numerically lower, so a naive "ORDER BY
-- exercise_id" backfill would get this session's order wrong.
-- Never edit this file after creation, same as v11-snapshot.sql.
...
INSERT INTO exercises (id, name, muscle_group, equipment, type, is_custom, modality, uuid) VALUES
  ...
  (2, 'Agachamento', 'legs', 'barbell', 'compound', 0, 'musculacao', 'fixed-uuid-ex-2');
...
-- Interleaved on purpose: exercise 2's first set (id=1) predates exercise 1's first
-- set (id=2), even though exercise_id 1 < exercise_id 2. The backfill must order this
-- session's exercises as [2, 1], matching MIN(id), not [1, 2] (numeric exercise_id).
INSERT INTO sets (id, session_id, exercise_id, set_number, reps, weight_kg, rpe, rir, notes, distance_km, duration_sec, pace_sec, failure) VALUES
  (1, 1, 2, 1, 10, 80, 8, 2, NULL, NULL, NULL, NULL, 0),
  (2, 1, 1, 1, 10, 60, 8, 2, NULL, NULL, NULL, NULL, 0),
  (3, 1, 2, 2, 8, 85, 8.5, 1, NULL, NULL, NULL, NULL, 0),
  (4, 1, 1, 2, 8, 65, 8.5, 1, NULL, NULL, NULL, NULL, 0);
